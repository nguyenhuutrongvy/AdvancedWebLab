...
namespace TatBlog.WebApi.Endpoints
{
    public static class CategoryEndpoints
    {
        ...
        {
            var categoriesList = await categoryRepository.GetCategoriesAsync(model.Name);
        ...
        private static async Task<IResult> GetPostsByCategoryId(int id, [AsParameters] PagingModel pagingModel, IBlogRepository blogRepository)
        {
        ...
        {
            return await categoryRepository.DeleteCategoryAsync(id) ? Results.NoContent() : Results.NotFound($"Could not find category with id = {id}");
        }
    }
}
